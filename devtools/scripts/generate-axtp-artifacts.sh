...
const { emitCppFiles } = await import(pathToFileURL(path.join(root, "devtools/generators/dist/emitters/cpp.js")).href);

const spec = await loadProtocolSources(specRoot);
for (const message of validateSpec(spec)) console.log(message);

const outDir = path.join(root, "include/core/protocol/generated");
await emitCppFiles(spec, outDir);
console.log(`[OK] generated C++ artifacts: ${outDir}`);
NODE

AXTP_SPEC_PATH="$spec" node "$root/devtools/scripts/axtp-versioning.mjs" generate --runtime-name axtp-cpp-runtime
